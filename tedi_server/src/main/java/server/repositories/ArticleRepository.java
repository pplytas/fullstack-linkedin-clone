...
public interface ArticleRepository extends JpaRepository<ArticleEntity, Long> {
	...
	@Query("SELECT a FROM ArticleEntity a "
			+ "WHERE "
			+ "a.user = ?1 "
			+ "OR (a.user IN (SELECT c1.user FROM ConnectionEntity c1 WHERE (c1.user = ?1 OR c1.connected = ?1) AND c1.isPending = false)) "
	...
	ArticleEntity findByTitleAndUserIsNull(String title);
